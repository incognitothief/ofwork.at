...
const IDENTIFIER_SECTION_IDS = [
  'identifiers',
  'id-iswc', 'id-isrc', 'id-ipi', 'id-grid', 'id-gtin',
  'id-isan', 'id-eidr',
  'id-isbn',
  'id-isni',
]

type Page = 'main' | 'identifiers'

const isIdentifiersId = (id: string) =>
  id === 'identifiers' || id.startsWith('id-')

const pageFromHash = (): Page =>
  isIdentifiersId(window.location.hash.slice(1)) ? 'identifiers' : 'main'
...
export default function App() {
  ...
  const prevPage = useRef<Page>(page)

  // Scroll to hash target after page switches
  useEffect(() => {
  ...
  useEffect(() => {
    const onHashChange = () => {
      ...
      const next = isIdentifiersId(id) ? 'identifiers' : 'main'
      setPage(next)
  ...
  // IntersectionObserver — re-run when page changes
  useEffect(() => {
    const ids = page === 'main' ? MAIN_SECTION_IDS : IDENTIFIER_SECTION_IDS
  ...
  const handleNavClick = useCallback((id: string) => {
    const next: Page = isIdentifiersId(id) ? 'identifiers' : 'main'
    setPage(next)
    setActiveId(id)
